...
import com.test.spring.boot.blog.entity.*;
...
public class BlogServiceImpl implements BlogService {
	...
	@Override
	@Transactional
	public Blog createComment(Long blogId, String commentContent,String gradeContent) {
		Blog originalBlog = blogRepository.findOne(blogId);
		User user = (User) SecurityContextHolder.getContext()
				.getAuthentication().getPrincipal();
//		Comment comment = new Comment(user, commentContent);
		int num = Integer.parseInt(gradeContent);
		Comment comment = new Comment(user, commentContent,blogId,num);
//		Blog blog = blogRepository.findOne(blogId);

		Grade grade = new Grade(user,gradeContent,blogId);
		originalBlog.addComment(comment);
		originalBlog.addGrade(grade);
		//System.out.print(originalBlog);
